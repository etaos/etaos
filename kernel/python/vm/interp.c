...
PmReturn_t interpret(const uint8_t returnOnNoThreads)
{
	...
	for (;;) {
		...
		case GET_ITER:
			...
			if (OBJ_GET_TYPE(TOS) == OBJ_TYPE_CLI) {
				retval =
				    class_getAttr(TOS, PM_NEXT_STR, &pobj1);
				if (retval != PM_RET_OK) {
					PM_RAISE(retval, PM_RET_EX_TYPE);
					break;
				}
		...
		case INPLACE_MULTIPLY:
			...
			/* If it's a string replication operation */
				...
				retval = string_replicate((uint8_t const
							   **)(uint8_t *) &
							  pobj2, t16, &pobj3);
				PM_BREAK_IF_ERROR(retval);
				PM_SP--;
		...
		case INPLACE_MODULO:
			...
			if (OBJ_GET_TYPE(TOS1) == OBJ_TYPE_STR) {
				retval =
				    string_format((pPmString_t) TOS1, TOS,
						  &pobj3);
				PM_BREAK_IF_ERROR(retval);
				PM_SP--;
				TOS = pobj3;
		...
		case SLICE_3:
			{
				...
				case OBJ_TYPE_STR:
					retval =
					    string_slice(pobj1, pstart, pend,
							 pstride, &pobj2);
					PM_BREAK_IF_ERROR(retval);
					TOS = pobj2;
					continue;
		...
		case RETURN_VALUE:
			...
			if (((pPmFrame_t) pobj1)->fo_func->f_co->
		...
		case STORE_NAME:
			...
			retval =
			    dict_setItem((pPmObj_t) PM_FP->fo_attrs, pobj2,
					 TOS);
			PM_BREAK_IF_ERROR(retval);
			PM_SP--;
			continue;
		...
		case DELETE_NAME:
			...
			retval =
			    dict_delItem((pPmObj_t) PM_FP->fo_attrs, pobj2);
			PM_BREAK_IF_ERROR(retval);
			continue;
#endif				/* HAVE_DEL */
		...
		case STORE_ATTR:
			...
			if ((OBJ_GET_TYPE(TOS) == OBJ_TYPE_FXN)
				...
				pobj2 =
				    (pPmObj_t) ((pPmInstance_t) TOS)->cli_attrs;
		...
		case STORE_GLOBAL:
			...
			retval =
			    dict_setItem((pPmObj_t) PM_FP->fo_globals, pobj2,
					 TOS);
			PM_BREAK_IF_ERROR(retval);
			PM_SP--;
			continue;
		...
		case DELETE_GLOBAL:
			...
			retval =
			    dict_delItem((pPmObj_t) PM_FP->fo_globals, pobj2);
			PM_BREAK_IF_ERROR(retval);
			continue;
#endif				/* HAVE_DEL */
		...
		case LOAD_NAME:
			...
			retval =
			    dict_getItem((pPmObj_t) PM_FP->fo_attrs, pobj1,
					 &pobj2);
			if (retval == PM_RET_EX_KEY) {
				/* Get val from globals */
				retval =
				    dict_getItem((pPmObj_t) PM_FP->fo_globals,
						 pobj1, &pobj2);
				...
				if ((retval == PM_RET_EX_KEY)
					...
					retval =
					    dict_getItem(PM_PBUILTINS, pobj1,
							 &pobj2);
					if (retval == PM_RET_EX_KEY) {
		...
		case IMPORT_FROM:
			...
			retval =
			    dict_getItem((pPmObj_t) ((pPmFunc_t) pobj1)->
					 f_attrs, pobj2, &pobj3);
			PM_BREAK_IF_ERROR(retval);
		...
		case LOAD_GLOBAL:
			...
			if (retval == PM_RET_EX_KEY) {
				retval =
				    dict_getItem(PM_PBUILTINS, pobj1, &pobj2);
		...
		case SETUP_LOOP:
			{
				...
				/* Create block */
				retval =
				    heap_getChunk(sizeof(PmBlock_t), &pchunk);
				PM_BREAK_IF_ERROR(retval);
		...
		case RAISE_VARARGS:
			...
			retval =
			    dict_getItem(PM_PBUILTINS, PM_EXCEPTION_STR,
					 &pobj2);
			if (retval != PM_RET_OK) {
				PM_RAISE(retval, PM_RET_EX_SYS);
				break;
			...
			retval =
			    dict_getItem((pPmObj_t) ((pPmClass_t) pobj1)->
					 cl_attrs, PM_CODE_STR, &pobj2);
			PM_BREAK_IF_ERROR(retval);

			/* Raise exception by breaking with retval set to code */
			PM_RAISE(retval,
				 (PmReturn_t) (((pPmInt_t) pobj2)->val & 0xFF));
			break;
#endif				/* HAVE_ASSERT */
		...
		case CALL_FUNCTION:
			...
			if ((OBJ_GET_TYPE(pobj1) == OBJ_TYPE_FXN)
				...
				sli_memcpy((uint8_t *) & ((pPmTuple_t) pobj2)->
					   val, (uint8_t *) & STACK(t16),
				...
				retval =
				    dict_getItem(PM_PBUILTINS, PM_GENERATOR_STR,
						 &pobj1);
			...
			if (OBJ_GET_TYPE(pobj1) == OBJ_TYPE_CLO) {
				...
				pobj3 = C_NULL;
				retval =
				    class_getAttr(pobj1, PM_INIT_STR, &pobj3);
				if (retval == PM_RET_EX_KEY) {
			...
			if (OBJ_GET_TYPE(pobj1) == OBJ_TYPE_MTH) {
				...
				STACK(t16) =
				    (pPmObj_t) ((pPmMethod_t) pobj1)->m_func;
 ...
 CALL_FUNC_FOR_ITER:
			...
			if (OBJ_GET_TYPE(((pPmFunc_t) pobj1)->f_co) ==
				...
				if (((pPmFunc_t) pobj1)->f_co->co_cellvars !=
				    C_NULL) {
					for (t8 = 0;
					     t8 <
					     ((pPmFunc_t) pobj1)->f_co->
					     co_cellvars->length; t8++) {
						if (((pPmInt_t)
						     ((pPmFunc_t) pobj1)->f_co->
						     co_cellvars->val[t8])->
						    val >= 0) {
							((pPmFrame_t) pobj2)->
							    fo_locals[((pPmFunc_t) pobj1)->f_co->co_nlocals + t8] = ((pPmFrame_t) pobj2)->fo_locals[((pPmInt_t) (((pPmFunc_t) pobj1)->f_co->co_cellvars->val[t8]))->val];
						}
					}
				}
				...
				for (t8 = 0;
				     t8 <
				     ((pPmFunc_t) pobj1)->f_co->co_nfreevars;
				     t8++) {
					C_ASSERT(((pPmFunc_t) pobj1)->
						 f_closure != C_NULL);
					((pPmFrame_t) pobj2)->
					    fo_locals[((pPmFunc_t) pobj1)->
						      f_co->co_nlocals +
						      ((((pPmFunc_t) pobj1)->
							f_co->co_cellvars ==
							C_NULL) ? 0
			...
			/* If it's native func */
				...
				gVmGlobal.nativeframe.nf_numlocals =
				    (uint8_t) t16;
				...
				if (t16 >= 0) {
					retval =
					    std_nat_fxn_table[t16] (&PM_FP);
				}

				/* Negative index is a usrlib func */
				else {
					retval =
					    usr_nat_fxn_table[-t16] (&PM_FP);
				}
 ...
 CALL_FUNC_CLEANUP:
		...
		case MAKE_FUNCTION:
			 ...
			 */
			retval =
			    func_new(TOS, (pPmObj_t) PM_FP->fo_globals, &pobj2);
			PM_BREAK_IF_ERROR(retval);
		...
		case LOAD_DEREF:
			...
			t16 = GET_ARG();
			pobj1 =
			    PM_FP->fo_locals[PM_FP->fo_func->f_co->co_nlocals +
					     t16];
			if (pobj1 == C_NULL) {
				PM_RAISE(retval, PM_RET_EX_SYS);
				break;
		...
		if (retval == PM_RET_EX_STOP) {
			...
			while ((retval == PM_RET_EX_STOP) && (pobj1 != C_NULL)) {
				pobj2 =
				    (pPmObj_t) ((pPmFrame_t) pobj1)->
				    fo_blockstack;
				while ((retval == PM_RET_EX_STOP)
				       && (pobj2 != C_NULL)) {
					if (((pPmBlock_t) pobj2)->b_type ==
					    B_LOOP) {
					...
					pobj2 =
					    (pPmObj_t) ((pPmBlock_t) pobj2)->
					    next;
				}
				pobj1 =
				    (pPmObj_t) ((pPmFrame_t) pobj1)->fo_back;
			}
			if (retval == PM_RET_OK) {
				continue;
			}
		...
		if ((gVmGlobal.threadList->length <= 1)
		    && (retval != PM_RET_OK)) {
			break;
		}
...
PmReturn_t interp_reschedule(void)
{
	...
	/* Otherwise, get the next thread in the list (round robin) */
		...
		retval =
		    list_getItem((pPmObj_t) gVmGlobal.threadList, threadIndex,
				 &pobj);
