...
sli_btoa16(uint8_t value, uint8_t * buf, uint8_t buflen, uint8_t upperCase)
{
	...
	buf[0] = (value >> 4) > 9
	    ? hexChars[value >> 4] + upperCase : hexChars[value >> 4];
	buf[1] = (value & 0x0F) > 9
	    ? hexChars[value & 0x0F] + upperCase : hexChars[value & 0x0F];
	buf[2] = '\0';
...
sli_ltoa16(int32_t value, uint8_t * buf, uint8_t buflen, uint8_t upperCase)
{
	...
	for (i = 28; i >= 0; i -= 4) {
		nibble = ((value >> i) & 0xF);
		if ((nibble == 0) && !showZero)
			continue;
		buf[j++] = (nibble > 9)
		    ? hexChars[nibble] + upperCase : hexChars[nibble];
		showZero = C_TRUE;
	}
	buf[j] = '\0';
...
sli_ptoa16(intptr_t value, uint8_t * buf, uint8_t buflen, uint8_t upperCase)
{
	...
	for (j = 0, i = 8 * sizeof(intptr_t) - 8; i >= 0; i -= 8, j += 2) {
		retval =
		    sli_btoa16((value >> i) & 0xFF, &buf[j], buflen - j,
			       upperCase);
