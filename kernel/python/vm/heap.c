...
#if 0
static void heap_gcPrintFreelist(void)
{
...
PmReturn_t heap_init(uint8_t * base, uint32_t size)
{
	...
	if ((hs % HEAP_MAX_FREE_CHUNK_SIZE) < HEAP_MIN_CHUNK_SIZE) {
		...
		pchunk =
		    (pPmHeapDesc_t) ((uint8_t *) pchunk + HEAP_MIN_CHUNK_SIZE);
	}

	/* Create as many max-sized chunks as possible in the freelist */
	for (; hs >= HEAP_MAX_FREE_CHUNK_SIZE; hs -= HEAP_MAX_FREE_CHUNK_SIZE) {
		OBJ_SET_FREE(pchunk, 1);
		CHUNK_SET_SIZE(pchunk, HEAP_MAX_FREE_CHUNK_SIZE);
		heap_linkToFreelist(pchunk);
		pchunk =
		    (pPmHeapDesc_t) ((uint8_t *) pchunk +
 ...
 * @return  Return code
 */
static PmReturn_t heap_gcMarkObj(pPmObj_t pobj)
{
	...
	case OBJ_TYPE_COB:
		...
		retval =
		    heap_gcMarkObj((pPmObj_t) ((pPmCo_t) pobj)->co_cellvars);
#endif				/* HAVE_CLOSURES */
		break;
	...
	case OBJ_TYPE_FXN:
		...
		retval =
		    heap_gcMarkObj((pPmObj_t) ((pPmFunc_t) pobj)->f_globals);
		PM_RETURN_IF_ERROR(retval);

#ifdef HAVE_DEFAULTARGS
		/* Mark the default args tuple */
		retval =
		    heap_gcMarkObj((pPmObj_t) ((pPmFunc_t) pobj)->
				   f_defaultargs);
		PM_RETURN_IF_ERROR(retval);
		...
		retval =
		    heap_gcMarkObj((pPmObj_t) ((pPmFunc_t) pobj)->f_closure);
#endif				/* HAVE_CLOSURES */
		break;
	...
	case OBJ_TYPE_CLI:
		...
		/* Mark the class */
		retval =
		    heap_gcMarkObj((pPmObj_t) ((pPmInstance_t) pobj)->
				   cli_class);
		PM_RETURN_IF_ERROR(retval);
	...
	case OBJ_TYPE_MTH:
		...
		/* Mark the instance */
		retval =
		    heap_gcMarkObj((pPmObj_t) ((pPmMethod_t) pobj)->m_instance);
		PM_RETURN_IF_ERROR(retval);

		/* Mark the func */
		retval =
		    heap_gcMarkObj((pPmObj_t) ((pPmMethod_t) pobj)->m_func);
		PM_RETURN_IF_ERROR(retval);

		/* Mark the attrs dict */
		retval =
		    heap_gcMarkObj((pPmObj_t) ((pPmMethod_t) pobj)->m_attrs);
		break;
	...
	case OBJ_TYPE_CLO:
		...
		/* Mark the attrs dict */
		retval =
		    heap_gcMarkObj((pPmObj_t) ((pPmClass_t) pobj)->cl_attrs);
		PM_RETURN_IF_ERROR(retval);

		/* Mark the base tuple */
		retval =
		    heap_gcMarkObj((pPmObj_t) ((pPmClass_t) pobj)->cl_bases);
		break;
#endif				/* HAVE_CLASSES */
	...
	case OBJ_TYPE_FRM:
		{
			...
			if ((((pPmFrame_t) pobj)->fo_func->f_co->
			     co_flags & CO_GENERATOR) == 0) {
				retval =
			...
			/* Mark the fxn obj */
			retval =
			    heap_gcMarkObj((pPmObj_t) ((pPmFrame_t) pobj)->
					   fo_func);
			PM_RETURN_IF_ERROR(retval);
	...
	case OBJ_TYPE_SGL:
		...
		for (i = 0; i < n; i++) {
			...
			retval =
			    heap_gcMarkObj(((pSegment_t) pobj)->
					   s_val[i % SEGLIST_OBJS_PER_SEG]);
			PM_RETURN_IF_ERROR(retval);
			...
			else if ((i % SEGLIST_OBJS_PER_SEG) ==
				 (SEGLIST_OBJS_PER_SEG - 1)) {
	...
	case OBJ_TYPE_THR:
		...
		retval =
		    heap_gcMarkObj((pPmObj_t) ((pPmThread_t) pobj)->pframe);
		break;
	...
	case OBJ_TYPE_NFM:
		...
		if (gVmGlobal.nativeframe.nf_active) {
			...
			for (i = 0; i < NATIVE_GET_NUM_ARGS(); i++) {
				retval =
				    heap_gcMarkObj(gVmGlobal.nativeframe.
						   nf_locals[i]);
				PM_RETURN_IF_ERROR(retval);
			}
		}
	...
	case OBJ_TYPE_BYA:
		...
		retval =
		    heap_gcMarkObj((pPmObj_t) ((pPmBytearray_t) pobj)->val);
		break;
 ...
 */
static PmReturn_t heap_gcMarkRoots(void)
{
	PmReturn_t retval;
	uint8_t i;
...
static PmReturn_t heap_purgeStringCache(uint8_t gcval)
{
	...
	for (pstr = *ppstrcache; pstr->next != C_NULL;) {
		...
		while ((pstr->next != C_NULL)
		       && (OBJ_GET_GCVAL(pstr->next) != gcval)) {
 ...
 */
static PmReturn_t heap_gcSweep(void)
{
	PmReturn_t retval;
	pPmObj_t pobj;
	pPmHeapDesc_t pchunk;
	...
	while ((uint8_t *) pobj < &pmHeap.base[pmHeap.size]) {
		...
		while (!OBJ_GET_FREE(pobj)
		       ...
		       && ((uint8_t *) pobj < &pmHeap.base[pmHeap.size])) {
			pobj =
			    (pPmObj_t) ((uint8_t *) pobj +
					PM_OBJ_GET_SIZE(pobj));
		}
		...
		while (OBJ_GET_FREE(pchunk)
			...
			totalchunksize =
			    totalchunksize + CHUNK_GET_SIZE(pchunk);

			C_DEBUG_PRINT(VERBOSITY_HIGH,
				      "heap_gcSweep(), id=%p, s=%d\n", pchunk,
				      CHUNK_GET_SIZE(pchunk));
